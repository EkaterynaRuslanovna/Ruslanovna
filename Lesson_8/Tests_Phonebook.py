...
class TestPhonebook(unittest.TestCase):
    ...
    def test_find_record_by_name(self):
        ...
        with self.assertRaises(Exception):
            self.phonebook.find_record_by_name("dfv")
    ...
    def test_find_record_by_surname(self):
        ...
        with self.assertRaises(Exception):
            self.phonebook.find_record_by_surname("dfvdv")
    ...
    def test_find_record_by_mobile_phone(self):
        ...
        with self.assertRaises(Exception):
            self.phonebook.find_record_by_mobile_phone("Non-existent Phone Number")

    def test_find_record_by_birthday(self):
        records = self.phonebook.find_record_by_birthday("20.05.95")
        self.assertEqual(len(records), 1)
        with self.assertRaises(Exception):
            self.phonebook.find_record_by_birthday("20.05.93")
    ...
    def test_delete_record(self):
        ...
        with self.assertRaises(Exception):
            self.phonebook.find_record_by_mobile_phone("1112223334")
        with self.assertRaises(Exception):
